...
fn main() {
    ...
    // Retrieve the new branch name from command line arguments
...
fn check_and_increment_version() {
    if is_path_exist("package.json") {
        ...
        // Trim the newline character from the input and update the version in the package object
        let new_version_number = new_version_number.trim();

        package["version"] = serde_json::Value::String(new_version_number.to_string());

        let updated_json = serde_json::to_string_pretty(&package).expect("Error serializing JSON");

        println!(
            "Updated package.json: {:?}",
            updated_json.trim().to_string()
        );
